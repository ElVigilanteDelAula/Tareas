...
long fibonacci(long numero){
    contador++;
	return numero = numero == 0||numero == 1 ? numero : fibonacci(numero-1) + fibonacci(numero-2);
}

long fibonacci_cached(long  numero){
    contador_cache++;
    long valor_en_cache = cache[numero];
    if(valor_en_cache <= 0 && numero != 0){
...
int main(int numero_de_argumentos, char **valores){
    ...
    cache[0]=0;
    cache[1]=1;
    posicion_a_calcular = atol(valores[1]);
    printf("la posicion numero %d de fibonacci es: %ld\n", posicion_a_calcular, fibonacci(posicion_a_calcular));
